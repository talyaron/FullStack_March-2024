...
function renderApp() {
    ...
    const html = renderHeader() + renderHomePage();
    app.innerHTML = html;
}

function renderHomePage() {
    const html = renderWelcome() + renderEvents() + renderPets() + renderContact();
    return html
}
...
function renderPet(pet: Pet) {
    const petCart = `
        <div class="pets-list__pet-cart" onclick="handleClick(${pet.id})">
...
function renderPetCart(pet: Pet) {
    const petCart = `
        <div class="pet-page">
    ...
    `;
    const app = document.querySelector('#app') as HTMLElement;
    app.innerHTML = renderHeader() + petCart;
}
