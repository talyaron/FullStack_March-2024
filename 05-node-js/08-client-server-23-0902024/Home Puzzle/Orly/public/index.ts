...
async function main(){
    try {
        const response = await fetch('http://localhost:3002/pets/get-all-pets');
    ...
    try {
       const form = e.target;
         const pet = {
            ...
            const response = await fetch('http://localhost:3002/pets/add-pet',{
                method: 'POST',
                headers: {
...
async function handleDeletePet(id:string){
    try {
        ...
        const response = await fetch(`http://localhost:3002/pets/delete-pet-id`,{
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function handlePriceUpdate(inputElement: HTMLInputElement, petId: string) {
    try {
        const newPrice = inputElement.valueAsNumber;

        if (isNaN(newPrice) || newPrice <= 0) {
            throw new Error('Invalid price value');
        }

        const response = await fetch(`http://localhost:3002/pets/update-price/${petId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (data.ok) {
            ...
            renderPets(data.pets, document.querySelector('#pets'));
        } else {
...
function filterDogs() {
    const dogPets = Pet.filter(pet => pet.species.toLowerCase() === 'dog');
    renderPets(dogPets, document.querySelector('#pets'));
}
