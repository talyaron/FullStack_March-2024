...
export function renderUpdateSubject(subjectDetails: Subject, id: string): string {
    ...
    const subject = `<label for="subject">Subject : </label>
                    <input type="text" name="subject" value="${subjectDetails.subject}">
                    <label for="grade">Grade : </label>
                    <input type="number" name="grade" value="${subjectDetails.score}">
                    <div class="show-buttons">
                        <div class="buttons buttons__update"><button name="update" id="${subjectDetails.id}@${id}">Update</button></div>
                    </div>`;
    return subject;
}
